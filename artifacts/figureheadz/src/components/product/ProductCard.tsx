    ...
    <Link href={`/product/${product.slug}`} className="group block relative h-full">
      <div className="bg-white comic-border comic-shadow rounded-none overflow-hidden h-full flex flex-col relative group-hover:border-primary transition-colors">
        ...
        <div className="absolute top-2 left-2 z-10 flex flex-col gap-2">
          {product.isNew && (
            <span className="bg-secondary text-white border-2 border-black font-display px-3 py-1 uppercase transform -rotate-2 drop-shadow-[2px_2px_0_#000] [text-shadow:-1px_-1px_0_#000,1px_-1px_0_#000,-1px_1px_0_#000,1px_1px_0_#000]">New!</span>
          )}
          {product.isExclusive && (
            <span className="bg-destructive text-white border-2 border-black font-display px-3 py-1 uppercase transform rotate-3 drop-shadow-[2px_2px_0_#000] [text-shadow:-1px_-1px_0_#000,1px_-1px_0_#000,-1px_1px_0_#000,1px_1px_0_#000]">Exclusive</span>
          )}
          {product.isOnSale && (
            <span className="bg-primary text-white border-2 border-black font-display px-3 py-1 uppercase transform -rotate-3 drop-shadow-[2px_2px_0_#000] [text-shadow:-1px_-1px_0_#000,1px_-1px_0_#000,-1px_1px_0_#000,1px_1px_0_#000]">Sale!</span>
          )}
        </div>
